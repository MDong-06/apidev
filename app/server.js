...
try {
    var i, j, n = 0;
    let srcPath = path.join(process.cwd(), "/scrApi/");
    ...
    for (var dir of getDirs) {
        fileName = readdirSync(path.join(process.cwd(), '/scrApi/' + dir + '/')).filter((file) => file.endsWith(".js") && file !== 'main.js');
        for (j of fileName) {
            var { index, name } = require(path.join(process.cwd(), '/scrApi/' + dir + '/') + j);
            router.get(name, index);
            n++
        }
    }
